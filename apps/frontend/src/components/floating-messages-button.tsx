...
function ChatPlusIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="h-4.5 w-4.5">
      <path strokeLinecap="round" strokeLinejoin="round" d="M8 11.5h5M8 15.5h8" />
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 20 7 17H5.5A2.5 2.5 0 0 1 3 14.5v-8A2.5 2.5 0 0 1 5.5 4h8" />
      <path strokeLinecap="round" strokeLinejoin="round" d="M17 6v6M14 9h6" />
    </svg>
  );
}
...
export function FloatingMessagesButton() {
  ...
  const unreadMessagesQuery = useQuery({
    queryKey: ["conversations", "floating-direct-unread"],
    queryFn: () =>
      apiFetch<ConversationSummary[]>("/conversations?kind=DIRECT&status=OPEN&unreadOnly=true", {
        token: accessToken ?? undefined
      }),
    enabled: Boolean(hasHydrated && accessToken && user && user.role !== "ADMIN")
  });
  ...
  const unreadConversations = unreadMessagesQuery.data ?? [];
  const unreadCount = unreadConversations.length;
  const latestUnread = unreadConversations[0] ?? null;

  const statusText = useMemo(() => {
    if (unreadMessagesQuery.isLoading) {
      return "Checking new messages...";
    }

    if (unreadCount > 0) {
      return `${unreadCount} unread ${unreadCount === 1 ? "message" : "messages"}`;
    }

    return "No unread direct messages";
  }, [unreadCount, unreadMessagesQuery.isLoading]);
  ...
  return (
    <div ref={containerRef} className="fixed bottom-24 right-5 z-50 flex flex-col items-end gap-3">
      <div
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h3 className="mt-2 text-lg font-semibold text-slate-950">Stay connected</h3>
        ...
        {latestUnread ? (
          <div className="mt-4 rounded-[22px] border border-sky-200 bg-sky-50/80 p-4">
            ...
            <p className="mt-2 text-sm font-semibold text-slate-900">
              {latestUnread.otherParticipant?.fullName ?? "Conversation"}
            </p>
            <p className="mt-1 line-clamp-2 text-sm leading-6 text-slate-600">
              {latestUnread.latestMessage?.body ?? "A new message is waiting for you."}
            </p>
            <Link
              href={`/messages?conversationId=${latestUnread.id}`}
              onClick={() => setOpen(false)}
              className="mt-3 inline-flex items-center gap-2 rounded-full border border-sky-200 bg-white px-3 py-2 text-xs font-semibold text-sky-700 transition hover:border-sky-300 hover:bg-sky-100"
            >
              <ReplyArrowIcon />
              Open latest chat
            </Link>
        ...
        <div className={`mt-5 grid gap-3 ${latestUnread ? "" : "sm:grid-cols-2"}`}>
          <Link
            href="/messages"
            onClick={() => setOpen(false)}
            className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:border-sky-300 hover:text-sky-700"
          >
            <InboxIcon />
            Inbox
          </Link>
      ...
      <button
        type="button"
        aria-label={open ? "Close messages panel" : "Open messages panel"}
        title="Messages"
        onClick={() => setOpen((current) => !current)}
        className={`group relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-sky-600 text-white shadow-[0_20px_45px_-18px_rgba(2,132,199,0.75)] transition duration-200 hover:-translate-y-1 hover:bg-sky-700 hover:shadow-[0_24px_55px_-18px_rgba(2,132,199,0.82)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-sky-200 ${
          unreadCount > 0 ? "message-launcher-pulse" : ""
        }`}
      >
        {unreadCount > 0 ? (
          <span className="pointer-events-none absolute inset-0 rounded-full border border-sky-300/80 message-launcher-ring" />
        ) : null}
        {unreadCount > 0 ? <MessageClosedIcon /> : <MessageOpenIcon />}
        {unreadCount > 0 ? (
