  ...
  return (
    <main className={`mx-auto w-full ${maxWidthClassName} px-3 py-4 sm:px-6 sm:py-7 lg:px-8 lg:py-10`}>
      <div className="surface-card-strong overflow-hidden rounded-[26px] p-4 sm:rounded-[32px] sm:p-6 lg:p-8">
        <div className={`border-b border-slate-200/80 pb-5 ${actionsInlineOnMobile ? "flex items-start justify-between gap-4 sm:flex-row sm:items-start sm:justify-between" : "flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between"}`}>
          ...
          {actions ? (
            <div className={`${actionsInlineOnMobile ? "shrink-0" : ""} flex flex-wrap gap-2 sm:justify-end`}>
              {actions}
            </div>
          ) : null}
        </div>
