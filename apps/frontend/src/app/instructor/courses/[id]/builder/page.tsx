          ...
          main={
            <>
              ...
              <WorkspacePanel
                ...
                description="Create draft Zoom or Google Meet sessions for this course, then schedule or complete them when the timing is right."
              >
                <div className="grid gap-6 lg:grid-cols-[minmax(0,320px)_minmax(0,1fr)]">
                  <form
                    onSubmit={(event) => {
                      event.preventDefault();
                      void builder.createInterview();
                    }}
                  ...
                  >
                    <input
                      ...
                      value={builder.interviewDraft.title}
                      onChange={(event) =>
                        builder.setInterviewDraft((current) => ({ ...current, title: event.target.value }))
                      }
                      required
                    />
                    <textarea
                      ...
                      value={builder.interviewDraft.description}
                      onChange={(event) =>
                        builder.setInterviewDraft((current) => ({ ...current, description: event.target.value }))
                      }
                    />
                    <select
                      className="rounded-2xl border border-slate-300 px-3 py-2 text-sm"
                      value={builder.interviewDraft.provider}
                      onChange={(event) =>
                        builder.setInterviewDraft((current) => ({
                          ...current,
                          provider: event.target.value as "ZOOM" | "GOOGLE_MEET"
                        }))
                      }
                    >
                    ...
                    <input
                      ...
                      value={builder.interviewDraft.meetingUrl}
                      onChange={(event) =>
                        builder.setInterviewDraft((current) => ({ ...current, meetingUrl: event.target.value }))
                      }
                      required
                    />
                    <input
                      type="datetime-local"
                      className="rounded-2xl border border-slate-300 px-3 py-2 text-sm"
                      value={builder.interviewDraft.scheduledAt}
                      onChange={(event) =>
                        builder.setInterviewDraft((current) => ({ ...current, scheduledAt: event.target.value }))
                      }
                      required
                    />
                    <input
                      ...
                      value={builder.interviewDraft.durationMinutes}
                      onChange={(event) =>
                        builder.setInterviewDraft((current) => ({
                          ...current,
                    ...
                    />
                    <PillButton type="submit">Create draft</PillButton>
                  </form>

                  <div className="space-y-3">
                    {(builder.interviewSessionsQuery.data ?? []).length ? (
                      (builder.interviewSessionsQuery.data ?? []).map((session) => (
                        <div key={session.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            ...
                            <StatusChip tone={session.status === "COMPLETED" ? "success" : session.status === "SCHEDULED" ? "info" : "warning"}>
                              {session.status}
                            </StatusChip>
                          </div>
                          <div className="mt-4 flex flex-wrap gap-2">
                            {session.status !== "SCHEDULED" ? (
                              <PillButton onClick={() => void builder.updateInterviewStatus(session.id, "SCHEDULED")}>
                                Schedule
                              </PillButton>
                            ) : null}
                            {session.status !== "COMPLETED" ? (
                              <PillButton onClick={() => void builder.updateInterviewStatus(session.id, "COMPLETED")}>
                                Mark completed
                              </PillButton>
                            ) : null}
                            {session.status !== "DRAFT" ? (
                              <PillButton onClick={() => void builder.updateInterviewStatus(session.id, "DRAFT")}>
                                Return to draft
                              </PillButton>
                            ) : null}
                            <PillButton onClick={() => void builder.deleteInterview(session.id)}>Delete</PillButton>
                          </div>
                        </div>
                      ))
                    ) : (
                      <EmptyState
                        ...
                        description="Create the first draft here, then schedule it when learners should join."
                      />
                    )}
                  </div>
                </div>
