...
interface AdminTenantsSectionProps {
  ...
  onActivateSubscription: (input: { tenantId: string; planId: string; billingPeriod: "MONTHLY" | "YEARLY"; isTrial?: boolean }) => void;
    ...
    <div className="mt-8 grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
      ...
      <ContentCard className="p-6">
        ...
        {tenantDetail ? (
          <div className="mt-4 space-y-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Invite Code: {tenantDetail.inviteCode}</p>
                <p className="mt-1 text-sm text-slate-500">
                  Current Plan: {tenantDetail.subscription?.currentSubscription?.plan.name ?? tenantDetail.subscription?.latestSubscription?.plan.name ?? "No plan assigned"}
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto]">
              ...
              <select
                ...
                onChange={(event) => setTenantBillingPeriod(event.target.value as "MONTHLY" | "YEARLY")}
                className="rounded-xl border border-slate-300 px-4 py-3 text-sm"
              >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
                ...
                <p className="mt-2">{tenantDetail.usage.usersCount} users</p>
                <p>{tenantDetail.usage.studentsCount ?? 0} students</p>
                <p>{tenantDetail.usage.instructorsCount ?? 0} instructors</p>
                <p>{tenantDetail.usage.adminsCount ?? 0} admins</p>
                <p>{tenantDetail.usage.coursesCount} courses</p>
                <p>{money.format(tenantDetail.usage.approvedRevenue ?? 0)} approved revenue</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-700">
                <p className="font-medium text-slate-900">Subscription</p>
                <p className="mt-2">
                  State: {tenantDetail.subscription?.currentSubscription?.state ?? tenantDetail.subscription?.latestSubscription?.state ?? "NONE"}
                </p>
                <p>Trial days remaining: {tenantDetail.subscription?.daysRemaining ?? 0}</p>
                <p>Freeze creation: {tenantDetail.subscription?.freezeCreation ? "Yes" : "No"}</p>
                <p>
                  Limit snapshot: {tenantDetail.plan?.maxCourses ?? tenantDetail.subscription?.trialRules.maxCourses ?? "—"} courses
                </p>
                <p>
                  Student cap: {tenantDetail.plan?.maxStudentsTotal ?? tenantDetail.subscription?.trialRules.maxStudentsTotal ?? "—"}
                </p>
              </div>
            </div>
          </div>
          ...
          <EmptyState title="Select a tenant" description="Choose a tenant from the list to inspect its plan and usage." />
        )}
      </ContentCard>
    </div>
  );
